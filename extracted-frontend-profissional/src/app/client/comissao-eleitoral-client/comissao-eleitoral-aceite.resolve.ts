...
export class ComissaoEleitoralAceiteResolve implements Resolve<any> {
  ...
  constructor(
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<any> {
        ...
        return new Observable(observer => {
          this.comissaoEleitoralService.getAceiteComissaoMembro(user.idProfissional).subscribe(
            data => {
              if(!data.isConnviteAceito || !data.situacaoCalendario){
                this.router.navigate(['/eleicao/convite']);
              }
              observer.next(data);
              observer.complete();
            },
