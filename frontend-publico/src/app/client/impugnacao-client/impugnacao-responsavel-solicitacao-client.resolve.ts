...
@Injectable()
export class ImpugnacaoRespnsavelSolicitacaoClientResolve implements Resolve<any> {
    ...
    resolve(route: ActivatedRouteSnapshot): Observable<any> {
        return new Observable(observer => {
          let id = route.params["id"];
            this.acompanharImpugnacaoClientService.pedidosImpugnacaoUfResponsavelSolicitante(id).subscribe(
                data => {
                    observer.next(data);
                    observer.complete();
                },
