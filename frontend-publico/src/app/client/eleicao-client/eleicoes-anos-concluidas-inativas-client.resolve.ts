...
export class EleicoesAnosConcluidasInativasClientResolve implements Resolve<any> {
   /**
   ...
   */
    resolve(route: ActivatedRouteSnapshot): Observable<any> {
        return new Observable(observer => {
          this.eleicaoClientService.getAnosFilter({"situacoes" : [Constants.ELEICAO_CONCLUIDA, Constants.ELEICAO_INATIVA]}).subscribe(
            data => {
              observer.next(data);
              observer.complete();
            },
