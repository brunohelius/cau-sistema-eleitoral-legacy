...
@Component({
    selector: 'app-modal-julgar-admissibilidade',
    templateUrl: './modal-julgar-admissibilidade.component.html',
    styleUrls: ['./modal-julgar-admissibilidade.component.scss']
})
export class ModalJulgarAdmissibilidadeComponent implements OnInit {
    ...
    public submit(form: NgForm) {
        this.submitted = true;
        if (form.valid) {
            this.denunciaService.submitJulgamentoAdmissibilidade(this.denuncia.idDenuncia, this.model).subscribe(data => {
                this.messageService.addMsgSuccess('MSG_JULGAMENTO_SUCESSO', [this.denuncia.numeroDenuncia]);
                setTimeout(() => {
