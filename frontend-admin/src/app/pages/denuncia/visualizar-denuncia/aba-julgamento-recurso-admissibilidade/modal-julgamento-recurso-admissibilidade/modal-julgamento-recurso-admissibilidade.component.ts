...
@Component({
  selector: 'app-modal-julgamento-recurso-admissibilidade',
  templateUrl: './modal-julgamento-recurso-admissibilidade.component.html',
  styleUrls: ['./modal-julgamento-recurso-admissibilidade.component.scss']
})
export class ModalJulgamentoRecursoAdmissibilidadeComponent implements OnInit {


    @Input() public denuncia;
    @Output() public cadastroRecursoJulgamentoAdmissibilidade: EventEmitter<any> = new EventEmitter();
    ...
    public tiposJulgamento: Array<{value, label}> = [
        {
            value: 1,
            label: 'LABEL_PROVIMENTO'
        },
        {
            value: 2,
            label: 'LABEL_IMPROVIMENTO'
        }
    ];
    ...
    ngOnInit() {
        ...
        this.model.recursoAdmissibilidade.id = this.denuncia.julgamentoAdmissibilidade.recursoJulgamentoAdmissibilidade.id;   
    }
    ...
    public submit(form: NgForm) {
        this.submitted = true;
        if (form.valid) {
            this.denunciaService.submitJulgamentoRecursoAdmissibilidade(this.model).subscribe(data => {
                this.messageService.addMsgSuccess('MSG_JULGAMENTO_RECURSO_ADMISSIBILIDADE', [this.denuncia.numeroDenuncia]);
                this.modalRef.hide();
                this.cadastroRecursoJulgamentoAdmissibilidade.emit(data);
