...
@Component({
  selector: 'modal-inadmitir-denuncia',
  templateUrl: './modal-inadmitir-denuncia.component.html',
  styleUrls: ['./modal-inadmitir-denuncia.component.scss']
})
export class ModalInadmitirDenunciaComponent implements OnInit {

  public configuracaoCkeditor: any = {};
  public arquivo: any;
  public inadmitir: any;
  ...
  ngOnInit() {
    this.inadmitir = this.getEstruturaDadosFormulario();
  ...
  public concluir = (form: NgForm) => {
    this.submitted = true;
    if (form.valid) {
      this.denunciaService.inadmitir(this.inadmitir).subscribe((data) => {
        this.messageService.addMsgSuccess('MSG_DENUNCIA_INADMITIDA_COM_EXITO', [this.nuDenuncia]);
        this.router.navigate(['denuncia/comissao/acompanhamento']);
        this.cancelarInadmitir();
      }, error => {
  ...
  /**
   * Fecha a modal de inadmitir
   */
  public cancelarInadmitir() {
  ...
  public getContagemDesignacaoRelator = () => {
    return Constants.TAMANHO_LIMITE_2000 - this.inadmitir.designacaoRelatorSimpleText.length;
  }
  ...
  public excluiUpload = (indice) => {
    this.arquivo = null;
    this.inadmitir.arquivoDenunciaInadmitida.splice(indice, 1);
  }
  ...
  public isQuantidadeUploadMaxima = () => {
    return this.inadmitir.arquivoDenunciaInadmitida.length == 5;
  }
  ...
  */
  public uploadDocumento = (arquivoEvent: any) => {
    ...
    if (this.inadmitir.arquivoDenunciaInadmitida.length < 5) {
      this.denunciaService.validarArquivoDenuncia(arquivoTO).subscribe(data => {
        this.inadmitir.arquivoDenunciaInadmitida.push(arquivoUpload);
 ...
 */
  ...
  */
  private setDescricaoDesignacaoRelatorSimpleText = (text: string) => {
    this.inadmitir.designacaoRelatorSimpleText = StringService.getPlainText(text).slice(0, -1);
  }
