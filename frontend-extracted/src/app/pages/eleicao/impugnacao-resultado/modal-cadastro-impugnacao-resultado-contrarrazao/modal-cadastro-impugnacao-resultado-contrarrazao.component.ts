    ...
    public inicializarContrarrazao(): void {
        if(this._contrarrazao) {
            this.contrarrazao = {
                ...
                arquivos: [{id: this._contrarrazao.id, nome: this._contrarrazao.nomeArquivo }],
   ...
   */
    ...
    public downloadArquivoContrarrazao(download: any): void {
        if(download.arquivo.id) {
            this.contrarrazaoService.download(download.arquivo.id).subscribe((data: Blob) =>{
    ...
    public salvar(form: NgForm): void {
        this.submitted = true;
        if (form.valid && this.hasArquivos()) {
