    ...
    */
    ...
    public voltar(): void {
        if(this.isMembroUf){
            ...
            this.router.navigate([`/eleicao/julgamento-final/acompanhar-ufs`]);
        }
    }

    /**
     * Redireciona para a página de pendencias da chapa
     * @param idChapa 
     */
    public redirecionaPendencias(idChapa: number): void{       
        this.router.navigate([
            `eleicao/julgamento-final/chapa-eleicao/listar-pendencias/${idChapa}`,{uf: this.cauUf.id}
        ]);
    }
     ...
     */
    public inicializarIES(): void {
        this.isIES = this.cauUf.id === Constants.ID_IES || this.cauUf.id === Constants.ID_CAUBR;
    }
    ...
    public getRotaDetalhamento(id) {
        let descricaoRota = this.isMembroUf ? 'membroComissaoUf': 'membroComissao';
        this.router.navigate([`/eleicao/julgamento-final/${descricaoRota}/chapaEleicao/${id}`,
        {
            isIES: this.inicializarIES()
        }]);
    }
    ...
    public atribuirImagemBandeira(): void {
        if(this.cauUf){
            ...
            this.inicializarIES();
            this.isMembroUf = false;
        } else {
            let idCauUf = this.getIdCauUfChapas();
    ...
    public getIdCauUfChapas(): any {
        let idCauUf = undefined;
        if( this.pedidos && this.pedidos.length > 0 ){
            idCauUf = this.pedidos[0].idCauUf;
        }
        return idCauUf;
    }
