...
export abstract class AbstractService {
  ...
  protected appendFormData(json: any, formData: FormData, firstKey?: string) {
    ...
    for (let key of Object.keys(json)) {
      let value = json[key];
      let formatedKey = firstKey !== undefined ? firstKey + '[' + key + ']' : key;

      if (value instanceof Array) {
        for (let index = 0; index < value.length; index++) {
          let formatedKeyArray = formatedKey + '[' + index + ']';

          if (typeof value === 'object') {
            if (typeof value[index] === 'string' || typeof value[index] === 'number' || value[index] instanceof File) {
